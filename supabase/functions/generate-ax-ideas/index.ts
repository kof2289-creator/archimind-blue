...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `당신은 제조산업 특히 업무 프로세스 자동화 및 전산화 및 데이터 통합에 대한 깊은 지식을 갖춘 전문 AI 전환(AX) 컨설턴트입니다. 귀하의 임무는 사용자가 제공한 '업무 영역', '고충 및 한계점', '기대 사항'을 직접적으로 해결하는 세 가지 뚜렷한 AI 솔루션 아이디어를 생성하는 것입니다. 각 아이디어는 다음 역할 중 하나에 해당해야 합니다:
...
- AX 도입을 통한 기대 사항: ${expectations}`;
                ...
                ideas: {
                  ...
                  items: {
                    ...
                    properties: {
                      solutionTitle: {
                        type: "string",
                        description: "솔루션에 대한 짧고 눈길을 끄는 제목. 예: 'AI 기반 실시간 품질 이상 감지 시스템'",
                      },
                      process: {
                        type: "string",
                        description: "이 솔루션이 적용될 간단한 업무 프로세스. 예: '데이터 입력 > AI 분석 > 결과 검토'",
                      },
                      category: {
                        type: "string",
                        enum: ["Assistant", "Advisor", "Agent"],
                        description: "솔루션의 역할 유형: Assistant, Advisor, 또는 Agent 중 하나.",
                      },
                      solutionOverview: {
                        type: "string",
                        description: "AX 솔루션 아이디어에 대한 간결한 1-3문장 요약.",
                      },
                      humanRole: {
                        type: "string",
                        description:
                          "이 새로운 프로세스에서 사람의 역할에 대한 간결한 설명. 예: 'AI가 감지한 이상 징후 검토 및 조치 승인'",
                      },
                      expectedEffects: {
                        type: "array",
                        items: { type: "string" },
                        description:
                          "3-5가지 주요 이점 목록. 짧은 구(2-4 단어)여야 합니다. 예: '품질 일관성 향상', '오류 감소', '업무 신속성 향상'",
                      },
                      keywords: {
                        type: "array",
                        items: { type: "string" },
                        description:
                          "3-5개의 관련 키워드 목록. 짧은 구여야 합니다. 예: '예측 분석', '실시간 모니터링', '의사결정 지원'",
                      },
                      technologies: {
                        type: "array",
                        items: { type: "string" },
                        description:
                          "3-5개의 관련 기술 목록. 예: '머신러닝 기반 예측 모델', '데이터 마이닝', '시뮬레이션'",
                      },
                    },
                    required: [
                      "solutionTitle",
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) {
        return new Response(JSON.stringify({ error: "사용량 한도를 초과했습니다. 잠시 후 다시 시도해주세요." }), {
          status: 429,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
    ...
    if (!toolCall || !toolCall.function?.arguments) {
      ...
      return new Response(JSON.stringify({ error: "아이디어를 생성하지 못했습니다" }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
