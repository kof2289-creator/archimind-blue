...
- AX 도입을 통한 기대 사항: ${expectations}`;
    ...
    console.log("Calling Google Gemini API for AX ideas generation");
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) {
        return new Response(JSON.stringify({ error: "API 사용량 한도를 초과했습니다. 잠시 후 다시 시도해주세요." }), {
          status: 429,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      ...
      if (response.status === 403) {
        return new Response(JSON.stringify({ error: "API 키가 유효하지 않거나 권한이 없습니다." }), {
          status: 403,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
    ...
    console.log("Ideas generated successfully:", ideas.length);
